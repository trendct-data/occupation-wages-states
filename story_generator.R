...
In ", state_name, ", there are about ", total_emp, " million full-time workers.


Most workers in the state are in major job category of ", most_workers_state$major_title, " (about ", most_workers_state$major_percent," percent). More specifically, the most employed people in the state work as ", most_workers_state$detailed_title," (", most_workers_state$detailed_percent," percent).
...
## Best paid jobs
", ifelse(substr(state_name, nchar(state_name), nchar(state_name))=="s", paste0(state_name, "'"), paste0(state_name, "'s"))," highest paid jobs include ", paste0(max_job_median$OCC_TITLE[2], ", ", max_job_median$OCC_TITLE[3], ", and ", max_job_median$OCC_TITLE[4]),".
               
About ", nrow(number1_median)," occupations in ", state_name," have the highest median annual pay compared to the other states in the country. Out of ", length(unique(state_wages$OCC_TITLE)),", that's ", round(length(number1_median)/length(unique(state_wages$OCC_TITLE))*100,0)," percent of all jobs with the highest pay categorized by the Bureau of Labor Statistics.
...
Its lowest-paying jobs include ", paste0(min_job_median$OCC_TITLE[4], ", ", min_job_median$OCC_TITLE[3], ", and ", min_job_median$OCC_TITLE[2]),". After adjusting for cost of living, those occupational groups make $", formatC(round(max(min_job_median$adjusted_A_MEAN[4])), format="d", big.mark=','),",  $", formatC(round(max(min_job_median$adjusted_A_MEAN[3])), format="d", big.mark=','),", and  $", formatC(round(max(min_job_median$adjusted_A_MEAN[2])), format="d", big.mark=','),", respectively.
               
", wages_all_state_min$OCC_TITLE[1]," have seen the largest decline in wages. Between ", wages_all_state_min$FIRST_YEAR[1]," and ", wages_all_state_min$LAST_YEAR[1],", the adjusted median annual salary dropped from $",  formatC(round(min(wages_all_state_min$FIRST_adjusted_A_MEDIAN[1])), format="d", big.mark=',')," to $",  formatC(round(min(wages_all_state_min$LAST_adjusted_A_MEDIAN[1])), format="d", big.mark=','),". That's a change of about ", wages_all_state_min$percent_change_wages[1]," percent.
               
Meanwhile, ", str_to_lower(wages_all_state_max$OCC_TITLE[5])," saw the sharpest increase in wages in ", state_name,". Wages went up ", wages_all_state_max$percent_change_wages[5]," percent between ", wages_all_state_max$FIRST_YEAR[1]," and ", wages_all_state_max$LAST_YEAR[1]," from $",  formatC(round(min(wages_all_state_max$FIRST_adjusted_A_MEDIAN[5])), format="d", big.mark=',')," to $",  formatC(round(min(wages_all_state_max$LAST_adjusted_A_MEDIAN[5])), format="d", big.mark=','),".

", growth_all_state_min$OCC_TITLE[1]," lost the most workers in ", state_name,". In ", growth_all_state_min$FIRST_YEAR[1],", there were ",  formatC(round(min(growth_all_state_min$FIRST_TOT_EMP[1])), format="d", big.mark=',')," employees. But by ", growth_all_state_min$LAST_YEAR[1],", that figure declined ", growth_all_state_min$percent_change_emp[1]*-1," percent to ",  formatC(round(min(growth_all_state_min$LAST_TOT_EMP[1])), format="d", big.mark=','),". 
