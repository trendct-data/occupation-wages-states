...
wages2015a <- subset(wages2015a, A_MEAN!=-1)

wages2015b <- subset(wages2015, !is.na(A_MEDIAN))
wages2015b <- subset(wages2015b, A_MEDIAN!=-1)
...
state_wages_median <- subset(state_wages_median, A_MEDIAN!=-1)

state_wages_mean <- state_wages %>%
  arrange(A_MEAN)
state_wages_mean <- subset(state_wages_mean, !is.na(A_MEAN))
state_wages_mean <- subset(state_wages_mean, A_MEAN!=-1)
...
state_wages_median <- subset(state_wages_median, adjusted_A_MEDIAN!=-1)
...
state_wages_mean <- subset(state_wages_mean, adjusted_A_MEAN!=-1)
...
The largest major job category in the state was <span class='occ_title_em state'>", most_workers_state$major_title, "</span> (about ", most_workers_state$major_percent," percent). Using narrower job descriptions, <span class='occ_title_em state'>", most_workers_state$detailed_title,"</span> were the largest category at ", most_workers_state$detailed_percent," percent.
               
The annual median pay ranges from $", formatC(round(min(state_wages_median$adjusted_A_MEDIAN)), format="d", big.mark=',')," for <span class='occ_title_em state'>", min_job_median$OCC_TITLE[1],"</span> at the lowest end of the spectrum to  $", formatC(round(max(state_wages_median$adjusted_A_MEDIAN)), format="d", big.mark=',')," for <span class='occ_title_em state'>", max_job_median$OCC_TITLE[5],"</span> at the highest. That's a gap of $", paste0(formatC(round(max(state_max_min_state$adj_diff)), format="d", big.mark=','),  ifelse(state_abbr=="MS", ", the largest in the country.", paste0(". Mississippi has the largest gap in the country of $193,000 between <span class='occ_title_em state'>anesthesiologists and psychiatric aides</span>."))),"
          
Credentials, experience, and skill contribute to the differences in pay within a given occupation.

## Best paid jobs
", ifelse(substr(state_name, nchar(state_name), nchar(state_name))=="s", paste0(state_name, "'"), paste0(state_name, "'s"))," highest paid jobs include <span class='occ_title_em state'>", paste0(max_job_median$OCC_TITLE[2], ", ", max_job_median$OCC_TITLE[3], "</span>, and <span class='occ_title_em state'>", max_job_median$OCC_TITLE[4]),"</span>.
